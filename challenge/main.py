...
def get_stock_combinations():

	stocks_list = list(filter(None, data))

	stock_combinations = itertools.combinations(stocks_list, 2)
...
stock_combinations = get_stock_combinations()

coeff_frame = pd.DataFrame()

for subset in stock_combinations:

	#read Adj close data for both stocks in a pair

	close0 = pd.read_csv(os.path.join('/home/luka/machine-learning/challenge/data/', subset[0] + '.csv'), usecols=['Adj Close'])
	close1 = pd.read_csv(os.path.join('/home/luka/machine-learning/challenge/data/', subset[1] + '.csv'), usecols=['Adj Close'])

	#renaming the column

	close0.rename(columns={'Adj Close': subset[0]}, inplace=True)
	close1.rename(columns={'Adj Close': subset[1]}, inplace=True)

	# applying the log returns formula

	close0 = close0.apply(lambda x: np.log(x) - np.log(x.shift(1)))
	close1 = close1.apply(lambda x: np.log(x) - np.log(x.shift(1)))

	#joining frames

	df3 = pd.concat([close0, close1], axis=1)

	df3.reindex()

	#the table with the stocks pair

	table = df3.corr(method='pearson')

	#selects the two stocks correlation from a table (we don't want to use the stock's correlation with itself)

	value = table.iloc[1,0]

	distance = math.sqrt(2*(1 - value))

	dictionary = pd.DataFrame({'stock_1': subset[0], 'stock_2': subset[1], 'corr_coeff': value, 'distance': distance}, index =[0])

	coeff_frame = coeff_frame.append(dictionary)

print(coeff_frame.to_string())
